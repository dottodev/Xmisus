...
public class DataReceiverTest {
    ...
    public void encodeFrame_roundTripsThroughFromBytes() {
        PlayerData p = new PlayerData(1, true, 55f, 66f, 1200f);
    ...
    @Test
    public void encodeFrame_marksAllyFlag() {
        PlayerData p = new PlayerData(1, false, 10f, 20f, 500f);
        byte[] frame = DataReceiver.encodeFrame(p);
        PlayerData decoded = PlayerData.fromBytes(frame);
        assertFalse(decoded.isEnemy);
        assertEquals(500f, decoded.hp, 0.001f);
    }
}
